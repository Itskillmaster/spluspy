...
for _name in ('InvokeAfterMsgsRequest', 'InvokeWithoutUpdatesRequest',
              'InvokeWithMessagesRangeRequest', 'InvokeWithTakeoutRequest'):
    try:
        _NESTS_QUERY.append(getattr(functions, _name))
    except AttributeError:
        pass
_NESTS_QUERY = tuple(_NESTS_QUERY)
...
class TimedOutError(RPCError):
    """
    Clicking buttons that never (or take too long to)
    call the callback answer will result in this "special" RPCError.
    """
