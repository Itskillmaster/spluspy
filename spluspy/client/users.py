    ...
    async def get_entity(
        ...
        for x in entity:
            if isinstance(x, str):
                ...
                try:
                    ...
                    _log.debug('Could not resolve entity %s, skipping', x)
        ...
        for x in inputs:
            if isinstance(x, str):
                ...
                pid = utils.get_peer_id(x, add_mark=False)
                if pid in id_entity:
                    result.append(id_entity[pid])
                else:
                    _log.debug('Entity with peer_id %s not found in results', pid)
            else:
                result.append(next(
        ...
        if single and not result:
            raise ValueError(
                'Could not find the input entity for the given argument')
            ...
            try:
                ...
                for user in contacts:
                    if user.id == peer.user_id:
                        return utils.get_input_peer(user)
            except errors.RPCError as e:
                _log.debug('Contacts fallback failed: %s', e)
