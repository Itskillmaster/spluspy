...
class ChatMethods:
    ...
    async def leave_chat(
        ...
        """
        Leaves a chat or channel.

        Arguments
            entity (`entity`):
                The chat or channel to leave.
        ...
        try:
            ...
            elif isinstance(entity, (types.InputPeerChat, types.InputChat)):
        ...
        # Fallback: scan dialogs to find the chat and extract its access_hash
        if isinstance(entity, int):
            target_id = abs(entity)
        elif isinstance(entity, str):
            target_id = None
        else:
            raise ValueError(
                'Could not resolve the entity and it is not an ID or username')

        async for dialog in self.iter_dialogs():
            if target_id is not None:
                if abs(dialog.id) == target_id:
                    break
            else:
                if dialog.name and dialog.name.lower() == entity.lower():
                    break
                if getattr(dialog.entity, 'username', None) \
                        and dialog.entity.username.lower() == entity.lower():
                    break
        else:
            raise ValueError(
                'Could not find the chat in the dialogs list. '
                'The bot might have already been kicked.')

        input_entity = dialog.input_entity
        if isinstance(input_entity, types.InputPeerChannel):
            channel = types.InputChannel(
                input_entity.channel_id, input_entity.access_hash)
            return await self(functions.channels.LeaveChannelRequest(channel))
        elif isinstance(input_entity, types.InputPeerChat):
            return await self(functions.messages.DeleteChatUserRequest(
                input_entity.chat_id, types.InputUserSelf()
            ))
        else:
            raise ValueError(
                'Found the dialog but it is neither a channel nor a chat')
