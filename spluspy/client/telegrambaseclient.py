...
class SoroushPlusBaseClient(abc.ABC):
    ...
    Arguments
        ...
        raise_last_call_error (`bool`, optional):
            When API calls fail in a way that causes spluspy to retry
    ...
    async def _keepalive_loop(self: 'SoroushClient'):
            """
            Dynamic Aggressive Polling: 0.3s normally, backs off safely on errors.
            """
            while self.is_connected():
                try:
                    ...
                    self._log[__name__].warning('FloodWait! سرور محدود کرد. توقف موقت برای %s ثانیه', e.seconds)
                    await asyncio.sleep(e.seconds)
                    
                except Exception as e:
                    self._log[__name__].debug('Aggressive polling failed: %s', e)
    ...
    async def _disconnect_coro(self: 'SoroushClient'):
        ...
        # Gracefully shut down the worker pool: drain pending updates,
        # cancel all worker tasks, and wait for them to finish.
        if self._worker_pool:
            # Wait for all queued updates to be processed
            await self._update_queue.join()
            # Cancel all worker tasks
            for worker in self._worker_pool:
                worker.cancel()
            if self._worker_pool:
                await asyncio.wait(self._worker_pool)
    ...
    async def _handle_auto_reconnect(self: 'SoroushClient'):
        """
        Default handler for auto-reconnect events.
        """
        pass
